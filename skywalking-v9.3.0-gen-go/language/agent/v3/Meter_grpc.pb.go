...
type MeterReportServiceClient interface {
	...
	// collected metrics in this period through one stream. The whole stream is an
	// input data set, client should onComplete the stream per report period.
	...
	// By using this, each one in the stream would be treated as a complete input
	// for MAL engine, comparing to `collect (stream MeterData)`, which is using
	// one stream as an input data set.
...
func (c *meterReportServiceClient) CollectBatch(ctx context.Context, opts ...grpc.CallOption) (MeterReportService_CollectBatchClient, error) {
	stream, err := c.cc.NewStream(ctx, &_MeterReportService_serviceDesc.Streams[1], "/skywalking.v3.MeterReportService/collectBatch", opts...)
	if err != nil {
		return nil, err
	}
...
func (x *meterReportServiceCollectBatchClient) CloseAndRecv() (*Commands, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(Commands)
...
func (x *meterReportServiceCollectBatchServer) SendAndClose(m *Commands) error {
	return x.ServerStream.SendMsg(m)
}
